...
class MathKt__MathJVMKt extends MathKt__MathHKt {
   ...
   public static final double log(double x, double base) {
      return !(base <= 0.0D) && base != 1.0D ? Math.log(x) / Math.log(base) : Double.NaN;
   }
   ...
   public static final double truncate(double x) {
      return !Double.isNaN(x) && !Double.isInfinite(x) ? (x > 0.0D ? Math.floor(x) : Math.ceil(x)) : x;
   }
   ...
   public static final int roundToInt(double $this$roundToInt) {
      if (Double.isNaN($this$roundToInt)) {
         ...
         return $this$roundToInt > 2.147483647E9D ? Integer.MAX_VALUE : ($this$roundToInt < -2.147483648E9D ? Integer.MIN_VALUE : (int)Math.round($this$roundToInt));
      }
   }
