...
/*
 * Safe memory operations for ACE bypass
...
/* Read process memory using KeStackAttachProcess */
...
/* Write process memory using KeStackAttachProcess */
...
template<typename T>
NTSTATUS ReadMemory(HANDLE pid, uintptr_t address, T* value)
{
    if (!value) return STATUS_INVALID_PARAMETER;
    
    NTSTATUS status = ReadProcessMemory(pid, (PVOID)address, value, sizeof(T));
...
template<typename T>
NTSTATUS WriteMemory(HANDLE pid, uintptr_t address, const T& value)
{
    NTSTATUS status = WriteProcessMemory(pid, (PVOID)address, (PVOID)&value, sizeof(T));
...
{
    PVOID ntoskrnlBase = GetProcessModuleBase((HANDLE)4, L"ntoskrnl.exe");
    if (!ntoskrnlBase) {
        return NULL;
    }
