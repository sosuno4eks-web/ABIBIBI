/*
 * hook.cpp - Function Pointer Hijack for Arena Breakout Infinite (ACE Bypass)
 *
...
{
    if (!handlerAddr) {
        DbgPrint("[!] Handler address is NULL\\n");
        return FALSE;
    }
    ...
    if (!hookTarget) {
        DbgPrint("[!] Target function not found in win32kbase.sys\\n");
        return FALSE;
    }
    
    DbgPrint("[+] Found target function at 0x%llX\\n", hookTarget);
    
    /* Store original function pointer */
    g_OriginalFunction = (OriginalFunctionPtr)hookTarget;
    
    /* Install function pointer hijack */
    if (InstallFunctionPointerHook(hookTarget, handlerAddr)) {
        DbgPrint("[+] Function pointer hijack installed successfully\\n");
        return TRUE;
    }
    
    /* Fallback: direct code patch if function pointer hook fails */
    DbgPrint("[!] Function pointer hook failed, trying direct patch\\n");
    ...
    DbgPrint("[+] Direct patch installed as fallback\\n");
    return TRUE;
}
...
BOOL InstallFunctionPointerHook(PVOID target, PVOID handler)
{
     ...
     * 2. Locate the target function pointer
    ...
    __try {
        ...
        *functionPtr = handler;
        ...
        DbgPrint("[+] Function pointer replaced successfully\\n");
        return TRUE;
    }
    __except (EXCEPTION_EXECUTE_HANDLER) {
        DbgPrint("[!] Function pointer replacement failed\\n");
        return FALSE;
    }
}

/* ── Hook Handler (Storage Communication) ─────────────────────────── */
...
NTSTATUS Hook::Handler(PVOID callParam)
{
    ...
    DbgPrint("[+] Processing storage request: %d\\n", req->Command);
    
    /* Process storage communication requests */
    NTSTATUS status = ProcessStorageRequest(req);
    
    if (NT_SUCCESS(status)) {
        DbgPrint("[+] Storage request completed successfully\\n");
    } else {
        DbgPrint("[!] Storage request failed: 0x%X\\n", status);
    }
...
NTSTATUS ProcessStorageRequest(PSTORAGE_COMM_BUFFER request)
{
    ...
    __try {
        switch (request->Command) {
            ...
            case 5: // Read GWorld
                {
                    PVOID gWorld = ReadGWorld((HANDLE)request->ProcessId);
                    if (gWorld) {
            ...
            case 6: // Read actor array
                {
                    ...
                    status = ReadActorArray((HANDLE)request->ProcessId, gWorld, &actorArray, &actorCount);
                    if (NT_SUCCESS(status)) {
    ...
    __except (EXCEPTION_EXECUTE_HANDLER) {
        ...
        request->Result = status;
        DbgPrint("[!] Exception in storage request processing\\n");
    }
...
VOID Hook::Cleanup()
{
    DbgPrint("[+] Cleaning up function pointer hijack\\n");
    ...
    if (g_OriginalFunction && g_HookTarget) {
        __try {
            ...
            *functionPtr = (PVOID)g_OriginalFunction;
            DbgPrint("[+] Original function pointer restored\\n");
        }
        __except (EXCEPTION_EXECUTE_HANDLER) {
            DbgPrint("[!] Failed to restore original function pointer\\n");
        }
    }
    
    DbgPrint("[+] Function pointer hijack cleanup completed\\n");
}
