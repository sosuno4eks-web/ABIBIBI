...
NTSTATUS RegistryFilter::Handler(PVOID callParam)
{
    ...
    PCOMMAND_PACKET req = (PCOMMAND_PACKET)callParam;
    ...
    __try {
        switch (request->Command) {
            ...
            case 3: // Get module base
                {
                    PVOID base = StorPortInitialize((HANDLE)request->ProcessId, request->ModuleName);
                    if (base && request->Buffer) {
...
/* Initialize storage communication - hook implementation */
extern "C" NTSTATUS InitializeStorageCommunication() 
{ 
    return RegistryFilter::Install(&RegistryFilter::Handler); 
}
