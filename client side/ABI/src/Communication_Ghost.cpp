...
extern "C" {
    ...
    NTSTATUS NTAPI SyscallStub(PMUTATED_PACKET packet, uint32_t syscallNumber);
}

// Implement syscall stub in inline assembly
__declspec(naked) NTSTATUS NTAPI SyscallStub(PMUTATED_PACKET packet, uint32_t syscallNumber) {
    __asm {
        mov r10, rcx                // Move packet pointer to r10
