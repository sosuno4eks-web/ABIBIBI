...
LONG WINAPI SimplestCrashHandler(EXCEPTION_POINTERS* ExceptionInfo)
{
	...
	if (rtkaudio_device_handle)
		intel_driver::Unload(rtkaudio_device_handle);
...
void help() {
	...
	Log(L"[+] Usage: rtkaudio.exe [--free][--mdl][--PassAllocationPtr] driver" << std::endl);
}
...
int wmain(const int argc, wchar_t** argv) {
	...
	rtkaudio_device_handle = intel_driver::Load();

	if (rtkaudio_device_handle == INVALID_HANDLE_VALUE)
		return -1;
	...
	NTSTATUS exitCode = 0;
	if (!kdmapper::MapDriver(rtkaudio_device_handle, raw_image.data(), 0, 0, free, true, kdmapper::AllocationMode::AllocatePool, false, callbackExample, &exitCode)) {
		Log(L"[-] Failed to map " << driver_path << std::endl);
		intel_driver::Unload(rtkaudio_device_handle);
		return -1;
	}

	if (!intel_driver::Unload(rtkaudio_device_handle)) {
