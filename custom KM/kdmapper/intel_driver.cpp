...
// Use fixed device name since embedded driver uses "Nal"
std::wstring GetDeviceNameW() {
	return L"Nal"; // Fixed to match embedded driver
}

std::wstring intel_driver::GetDriverNameW() {
	if (cachedDriverName.empty()) {
		// Create a realistic audio driver name
		static const char* prefixes[] = {
			"Rtk", "Realtek", "Intel", "Conexant", "VIA"
		};
		static const char* suffixes[] = {
			"Audio", "HDAudio", "AudioHD", "Sound", "AC97"
		};
		...
		int suffix_idx = rand() % (sizeof(suffixes) / sizeof(suffixes[0]));
		int version_idx = rand() % (sizeof(versions) / sizeof(versions[0]));
		
		sprintf_s(buffer, "%s%s%s", prefixes[prefix_idx], suffixes[suffix_idx], versions[version_idx]);
...
bool intel_driver::IsRunning() {
	std::wstring devicePath = L"\\\\.\\" + GetDeviceNameW();
	const HANDLE file_handle = CreateFileW(devicePath.c_str(), FILE_ANY_ACCESS, 0, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);
...
NTSTATUS intel_driver::Load() {
	...
	//from https://github.com/ShoaShekelbergstein/rtkaudio as some Drivers takes same device name
	if (intel_driver::IsRunning()) {
		kdmLog(L"[-] \\Device\\" << GetDeviceNameW() << L" is already in use." << std::endl);
		kdmLog(L"[-] This means that there is a Realtek Audio Driver already loaded or another instance of rtkaudio is running or rtkaudio crashed and didn't unload the previous driver." << std::endl);
		kdmLog(L"[-] If you are sure that there is no other instance of rtkaudio running, you can try to restart your computer to fix this issue." << std::endl);
		kdmLog(L"[-] If the problem persists, you can try to unload the Realtek Audio Driver manually (If the driver was loaded with rtkaudio will have a random name and will be located in %temp%), if not, the driver name is rtkaudio_fix.sys." << std::endl);
	...
	std::wstring devicePath = L"\\\\.\\" + GetDeviceNameW();
	hDevice = CreateFileW(devicePath.c_str(), GENERIC_READ | GENERIC_WRITE, 0, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);

	if (!hDevice || hDevice == INVALID_HANDLE_VALUE)
	{
		kdmLog(L"[-] Failed to load driver rtkaudio_fix.sys" << std::endl);
		intel_driver::Unload();
