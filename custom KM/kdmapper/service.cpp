...
NTSTATUS service::RegisterAndStart(const std::wstring& driver_path, const std::wstring& serviceName) {
	...
	if (ntStatus == STATUS_IMAGE_CERT_REVOKED) {
		kdmLog("[-] Your vulnerable driver list is enabled and have blocked the driver loading, you must disable vulnerable driver list to use rtkaudio with Realtek Audio Driver" << std::endl);
