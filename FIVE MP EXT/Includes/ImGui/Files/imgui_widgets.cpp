...
struct button_state {
	float hover_alpha = 0.f; // <- adicione isso
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
	...
	if (size.x <= 0.0f)
		size.x = GetWindowContentRegionMax().x - GetWindowContentRegionMin().x - (window->ScrollbarY ? 5.f : 0.f);
	if (size.y <= 0.0f)
		size.y = 28.f;
	...
	// Animações de alpha e cor do texto
	it_anim->second.label_color = ImLerp(it_anim->second.label_color,
		it_anim->second.active ? colors::button_label_active : colors::button_label_inactive,
		g.IO.DeltaTime * 12.f);
	it_anim->second.alpha = ImClamp(it_anim->second.alpha + (6.f * g.IO.DeltaTime * (it_anim->second.active ? 1.f : -1.f)), 0.f, 1.f);

	// Suavização do hover (fundo)
	float target_hover_alpha = hovered ? 1.0f : 0.0f;
	it_anim->second.hover_alpha = ImLerp(it_anim->second.hover_alpha, target_hover_alpha, g.IO.DeltaTime * 10.0f);

	// Interpolação da cor de fundo
	ImU32 normal_col = GetColorU32(colors::button_background);
	ImU32 hover_col = IM_COL32(20, 20, 25, 255);

	ImVec4 col_normal = ColorConvertU32ToFloat4(normal_col);
	ImVec4 col_hover = ColorConvertU32ToFloat4(hover_col);
	ImVec4 final_col = ImLerp(col_normal, col_hover, it_anim->second.hover_alpha);

	window->DrawList->AddRectFilled(rect.Min, rect.Max, ColorConvertFloat4ToU32(final_col), 4.f);
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
	...
	window->DrawList->AddRectFilled(grab_rect.Min, grab_rect.Max,
		GetColorU32(ImVec4(accent_color[0], accent_color[1], accent_color[2], accent_color[3])), 8.f);
	return held;
}
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, int val, ImGuiComboFlags flags, bool multi)
{
	...
	it_anim->second.expand_color = ImLerp(
		it_anim->second.expand_color,
		it_anim->second.combo_opened ?
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
	...
	struct Slider_t {
		ImVec4 BackGroundCol = ImColor(23, 23, 30);
		float Slow;
	};
	...
	SliderAnim->second.BackGroundCol = ImLerp(SliderAnim->second.BackGroundCol, Hovered ? ImColor(23, 23, 30) : ImColor(23, 23, 30), g.IO.DeltaTime * 6.f);

	Window->DrawList->AddRectFilled(SliderRect.Min, SliderRect.Max, ImGui::GetColorU32(SliderAnim->second.BackGroundCol), 5);
	Window->DrawList->AddRect(SliderRect.Min, SliderRect.Max, ImGui::GetColorU32(ImVec4(ImColor(23, 23, 30))), 5);
	Window->DrawList->AddRectFilled(
		ImVec2(SliderRect.Min.x, SliderRect.Min.y),
		ImVec2(SliderRect.Min.x + SliderAnim->second.Slow, SliderRect.Max.y),
		ImGui::GetColorU32(ImVec4(accent_color[0], accent_color[1], accent_color[2], accent_color[3])),
		5
	);
	float thumb_width = 20.0f;  //largura
	float thumb_radius = 4.0f;  //raio
	float thumb_height = (SliderRect.Max.y - SliderRect.Min.y) + 2.0f;  //altura


	float center_y = (SliderRect.Min.y + SliderRect.Max.y) * 0.5f;
	float thumb_top = center_y - thumb_height * 0.5f;
	float thumb_bottom = center_y + thumb_height * 0.5f;


	ImVec2 thumb_pos = ImVec2(SliderRect.Min.x + SliderAnim->second.Slow - thumb_width / 2.0f, thumb_top);
	ImVec2 thumb_end = ImVec2(thumb_pos.x + thumb_width, thumb_bottom);


	thumb_pos.x = ImClamp(thumb_pos.x, SliderRect.Min.x, SliderRect.Max.x - thumb_width);
	thumb_end.x = ImClamp(thumb_end.x, SliderRect.Min.x + thumb_width, SliderRect.Max.x);


	Window->DrawList->AddRectFilled(thumb_pos, thumb_end, ImGui::GetColorU32(ImVec4(1.0f, 1.0f, 1.0f, 1.0f)), thumb_radius);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
	...
	it_anim->second.background = ImLerp(it_anim->second.background, IsItemActive() || hovered ? ImColor(23, 23, 30, 250) : ImColor(23, 23, 30, 250), g.IO.DeltaTime * 4.f);
	...
	if (!is_multiline)
	{
		...
		// Preenchimento com cantos arredondados (6.f)
		draw_list->AddRectFilled(frame_bb.Min, frame_bb.Max, GetColorU32(it_anim->second.background), 6.f);

		// Borda com espessura 1, arredondamento 3, cor (25, 25, 25, 255)
		draw_list->AddRect(frame_bb.Min, frame_bb.Max, IM_COL32(23, 23, 30, 255), 3.f, 0, 1.f);

	}

	const ImVec4 clip_rect(frame_bb.Min.x, frame_bb.Min.y, frame_bb.Min.x + inner_size.x, frame_bb.Min.y + inner_size.y); // Not using frame_bb.Max because we have adjusted size
	ImVec2 draw_pos = is_multiline ? draw_window->DC.CursorPos : frame_bb.Min + ImVec2(10, 4);
	ImVec2 text_size(10.0f, 4.0f);
	...
	if (render_cursor || render_selection)
	{
		...
		if (render_selection)
		{
			...
			ImU32 bg_color = ImColor(87, 83, 78, 40); // FIXME: current code flow mandate that render_cursor is always true here, we are leaving the transparent one for tests.
...
bool ImGui::CustomSelectableEx(const char* label, bool active)
{
	...
	it_anim->second = ImLerp(it_anim->second, active ? ImVec4(accent_color[0], accent_color[1], accent_color[2], accent_color[3]) : colors::checkbox_label_inactive, g.IO.DeltaTime * 8.f);
...
bool ImGui::Keybind(const char* label, int* key, int* mode)
{
	...
	window->DrawList->AddRectFilled(ImVec2(rect.Max.x - it_anim->second.PosX, rect.Min.y), ImVec2(rect.Max.x + 6, rect.Max.y), GetColorU32(ImVec4(ImColor(23, 23, 30))), 4);
...
bool ImGui::ListSelectableEx(const char* Label, bool Selected)
{
	...
	struct Selectable_t {
		...
		ImVec4 BackGroundCol = (ImVec4)ImColor(23, 23, 30);
	...
	SelectableWidget->second.BackGroundCol = ImLerp(SelectableWidget->second.BackGroundCol, Selected || Hovered ? (ImVec4)ImColor(23, 23, 30) : (ImVec4)ImColor(23, 23, 30), g.IO.DeltaTime * 8.f);
	SelectableWidget->second.CircleCol = ImLerp(
		SelectableWidget->second.CircleCol,
		Selected || Hovered ?
		ImVec4(accent_color[0], accent_color[1], accent_color[2], accent_color[3]) :
	...
	Window->DrawList->AddRect(Rect.Min, Rect.Max, GetColorU32(ImVec4(ImColor(23, 23, 30))), 6);
