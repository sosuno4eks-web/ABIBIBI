...
NTSTATUS SyncMemory(HANDLE pid, PVOID src, PVOID dst, SIZE_T size, BOOLEAN write)
{
    ...
    __try {
        // Use MmCopyVirtualMemory for ALL operations (most stable for Win10 22H2)
        ...
        // Fallback to KeStackAttachProcess only if MmCopyVirtualMemory fails
        KAPC_STATE apcState;
...
PVOID DiskControllerGetModuleBase(HANDLE processId)
{
    return DiskSectorGetModuleBase(processId);
}

NTSTATUS SataReadSector(HANDLE processId, PVOID address, PVOID buffer, SIZE_T size, BOOLEAN isWrite)
{
    return StorageSafeCopy(processId, address, buffer, size, isWrite);
}


/* ── GetSystemModuleBase Implementation ───────────────────────── */
...
PVOID GetSystemModuleBase(const char* moduleName)
{
    ...
    __try {
        ...
        PSYSTEM_MODULE_INFORMATION_EX moduleInfo = (PSYSTEM_MODULE_INFORMATION_EX)
            ExAllocatePoolWithTag(NonPagedPool, size, 'domS');
        
        if (!moduleInfo) {
            return NULL;
        }

        NTSTATUS status = ZwQuerySystemInformation(SystemModuleInformation, moduleInfo, size, &size);
        if (!NT_SUCCESS(status)) {
            ExFreePoolWithTag(moduleInfo, 'domS');
            return NULL;
        }

        PVOID moduleBase = NULL;
        for (ULONG i = 0; i < moduleInfo->ModulesCount; i++) {
            PSYSTEM_MODULE_INFORMATION module = &moduleInfo->Modules[i];
            
            // Compare module name (case-insensitive)
            if (_stricmp((const char*)module->ImageName + module->ModuleNameOffset, moduleName) == 0) {
